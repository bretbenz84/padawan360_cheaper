...
#define PTURNSERVOMIN 180     // adjust Periscope  Turn position (150 - 600)
#define PTURNSERVOFIRST 270   // adjust Periscope first turn position
#define PTURNSERVOSECOND 360  // adjust Periscope second turn position
#define PTURNSERVOMAX 450     // adjust Periscope  Turn position
...
#define LFTURNSERVOMIN 180    // adjust Life Form Scanner Turn position (150 - 600)
#define LFTURNSERVOMAX 500    // adjust Life Form Scanner Turn position
...
// Motor drivers
#define PIN1 10    //Periscope Motor Driver IN1
#define PIN2 11    //Periscope Motor Driver IN2
...
#define LSIN1 12  //Lightsaber Motor Driver IN1
#define LSIN2 13  //Lightsaber Motor Driver IN2
#define LFIN1 2  //Lifeform Motor Driver IN1
#define LFIN2 3  //Lifeform Motor Driver IN2
#define DSIN1 8   //Drink Server Motor Driver IN1
#define DSIN2 9   //Drink Server Driver IN2
//Limit Switches for up and down
#define PTop 30   //Periscope top limit switch
#define PBot 31   //Periscope bottom limit switch
...
#define LFTop 22  //Lifeform top limit switch
#define LFBot 23  //Lifeform bottom limit switch
...
static enum { ZAP_MOVE_TOP,
              ZAP_TOP } statezapup;  //zapper up
static enum { ZAP_MOVE_BOT,
              ZAP_BOT } statezapdown;  // zapper down
static enum { P_MOVE_TOP,
              P_TOP } statepup;  //periscope up
static enum { P_MOVE_BOT,
              P_BOT } statepdown;  //periscope down
static enum { LF_MOVE_TOP,
              LF_TOP } statelfup;  //lifeform scanner up
static enum { LF_MOVE_BOT,
              LF_BOT } statelfdown;  //lifeform scanner down
static enum { BM_MOVE_TOP,
              BM_TOP } statebmup;  //Bad Motivator up
static enum { BM_MOVE_BOT,
              BM_BOT } statebmdown;  //Bad Motivator down
static enum { LS_MOVE_TOP,
              LS_TOP } statelsup;  //Lightsaber up
static enum { LS_MOVE_BOT,
              LS_BOT } statelsdown;  //Lightsaber down
static enum { DS_MOVE_TOP,
              DS_TOP } statedsup;  //Drink Server up
static enum { DS_MOVE_BOT,
              DS_BOT } statedsdown;  //Drink Server down
...
bool virtualButton0 = false;
bool virtualButton1 = false;
bool virtualButton2 = false;
bool virtualButton3 = false;
bool virtualButton4 = false;
...
void loop() {
  ...
  // buttonState  - the current state of the button for Dome Zapper      - SerialLn 3
  // buttonState1 - the current state of the button for Periscope.       - SerialLn 4
  // buttonState2 - the current state of the button for Lifeform Scanner - SerialLn 1
  // buttonState3 - the current state of the button for Bad Motivator    - SerialLn 2
  // buttonState4 - the current state of the button for Lightsaber
  // buttonState5 - the current state of the button for Drink Server



  // Handle Serial2 input
  if (Serial2.available() > 0) {
    int channel = Serial2.parseInt();  // Read the incoming channel number (0-5)
    if (channel >= 0 && channel < 6) {
      // Set virtual button flags based on the channel
      if (channel == 4) {
        virtualButton1 = true;
      } else if (channel == 1) {
        virtualButton2 = true;
  ...
  // Process virtual buttons (just like real button presses)
  if (virtualButton0) {
    buttonPushCounter++;
    virtualButton0 = false;  // Reset after processing
  }
  if (virtualButton1) {
    buttonPushCounter1++;
    virtualButton1 = false;  // Reset after processing
  }
  if (virtualButton2) {
    buttonPushCounter2++;
    virtualButton2 = false;  // Reset after processing
  }
  if (virtualButton3) {
    buttonPushCounter3++;
    virtualButton3 = false;  // Reset after processing
  }
  if (virtualButton4) {
    buttonPushCounter4++;
    virtualButton4 = false;  // Reset after processing
  }
...
void PeriscopeTurn() {  // turn the periscope back and forwards
  switch (statept) {
    case 0: // Move to MAX position
      currentMillis = millis();
      if (currentMillis - pturnpreviousMillis >= pturninterval) {
        pwm.setPWM(6, 0, PTURNSERVOMAX);
        pturnpreviousMillis = currentMillis;
        statept = 1;
      }
      break;

    case 1: // Move to SECOND position
      currentMillis = millis();
      if (currentMillis - pturnpreviousMillis >= pturninterval) {
        pwm.setPWM(6, 0, PTURNSERVOSECOND);
        pturnpreviousMillis = currentMillis;
        statept = 2;
      }
      break;

    case 2: // Move to FIRST position
      currentMillis = millis();
      if (currentMillis - pturnpreviousMillis >= pturninterval) {
        pwm.setPWM(6, 0, PTURNSERVOFIRST);
        pturnpreviousMillis = currentMillis;
        statept = 3;
      }
      break;

    case 3: // Move to MIN position
      currentMillis = millis();
      if (currentMillis - pturnpreviousMillis >= pturninterval) {
        pwm.setPWM(6, 0, PTURNSERVOMIN);
        pturnpreviousMillis = currentMillis;
        statept = 4;
      }
      break;

    case 4: // Increment cycle count and decide whether to repeat or finish
      currentMillis = millis();
      pturncount++;
      if (pturncount == 2) { // Repeat 2 cycles
        statept = 5;         // Transition to stopping state or next logic
        pturncount = 0;
      } else {
        statept = 0; // Start over
      }
      break;
  }
}
